...
typedef struct {
    NanoJob buffer[NANO_QUEUE_SIZE]; // Le tapis physique contenant nos tickets
    
    _Atomic uint64_t head; // L'index de lecture (géré par les Forgerons)
    uint64_t tail;         // L'index d'écriture (géré UNIQUEMENT par Atropos)
} AtroposRingBuffer;
