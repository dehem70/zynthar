...
@app.get("/api/world/chunk/{x}/{y}/heightmap.png")
def get_chunk_heightmap_png(x: int, y: int):
    try:
        # Image de 101x101 pixels pour inclure les lignes de couture
        img = Image.new("L", (101, 101)) 
        pixels = img.load()

        seed = generator.world_seed

        for row in range(101):
            for col in range(101):
                # 1. COORDONNÉES GLOBALES CONTINUES
                global_x = float(x * 100 + col)
                
                # INVERSION DE SÉCURITÉ : On inverse l'index de la ligne (100 - row) 
                # pour que le haut de l'image (row=0) corresponde au Y le plus élevé en 3D.
                global_y = float(y * 100 + (100 - row))

                # 2. CALCUL DU BRUIT DE PERLIN
                from noise import pnoise2
                val = pnoise2(global_x / 200.0, global_y / 200.0, octaves=3, base=seed)
                
                # Normalisation entre 0 et 255
                pixel_val = int((val + 1.0) / 2.0 * 255)
                pixel_val = max(0, min(255, pixel_val))

                # 3. ÉCRITURE STRICTE : [colonne, ligne] -> [X, Y]
                # Une erreur ici inverse le sens de la map
                pixels[col, row] = pixel_val
